...
const CategorySelector = () => {
  ...
  const filteredProducts = products.filter(p => {
    if (selectedCategory === 'bestseller') {
      return p.category === 'fruits' && p.tag && p.tag.toLowerCase().includes('bestseller');
    }
  ...
  return (
    <div className="my-2 w-full flex flex-col items-center max-w-6xl mx-auto">
      ...
      {!selectedCategory && (
        <div className="w-full max-w-6xl px-4 mt-8">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-600 text-sm">Discover our best products from each category</p>
          </div>
          
          {/* Show 2 products from each category */}
          <div className="grid grid-cols-2 gap-x-2 gap-y-4 mb-6">
            {categories.slice(0, 2).map(category => {
              ...
              return categoryProducts.map(product => (
                ...
                >
                  {/* Category badge */}
                  <span className="absolute top-2 left-2 bg-blue-100 text-blue-600 text-xs font-bold px-2 py-1 rounded-full z-10 shadow">
                    {category.name}
                  </span>
      ...
      {/* Filtered Products - Show when category is selected */}
      {selectedCategory && (
        <div className="w-full max-w-6xl px-4">
          {/* Section Title */}
          <div className="text-center mb-6">
            <h2 className="text-2xl font-bold text-gray-800 mb-2">
              {selectedCategory === 'bestseller' ? 'Bestseller Fruits' : categories.find(cat => cat.id === selectedCategory)?.name + ' Products'}
            </h2>
            {selectedSubcategory && selectedCategory !== 'bestseller' && (
